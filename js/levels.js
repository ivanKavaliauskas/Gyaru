...
function construirPlataformas(scene) {
  ...
  if (
    currentLevel !== 4 &&
    currentLevel !== 8 &&
    posiblesPlataformas.length > 0
  ) {
    let platElegida =
      posiblesPlataformas[
        Phaser.Math.Between(0, posiblesPlataformas.length - 1)
      ];
    let margen = 20;
    let randomX = Phaser.Math.Between(
      platElegida.x - platElegida.ancho / 2 + margen,
      platElegida.x + platElegida.ancho / 2 - margen,
    );

    // 🌟 CORREGIDO: Añadimos "scene." antes de "items" para usar el grupo correcto de la escena activa
    let cherry = scene.items.create(
      randomX,
      platElegida.y - 25,
      "cherry_fruit",
    );
...
function recibirDano(p, origin) {
  if (!isInvincible && !isGameOver) {
    ...
    if (lives <= 0) {
      ...
      isInvincible = true;
      p.setVelocityY(-300)
        .setVelocityX(p.x < origin.x ? -250 : 250)
        .setTint(0xffffff);
